...
$('.show_test').click(()=> { // выводим тест на экран по клику на кнопку
	$('.show_test').hide();
	$('.check').css("display", "block");
    $('.check').before(page);
});

$('.check').on('click', ()=> { //проверяем результаты по клику на кнопку
    let $form = $('form')[0];
    let result = true;
    for (let j = 0; j < questions.length; j++){
				let questionIndex = parseInt($form[questions[j].id].value);
				let valid = trueAnswers[questions[j].id] === questionIndex;
        console.log(valid);
        if (!valid){
            result = false;
            break;
